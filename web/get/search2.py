...
def get_search(request):
    ...
    if total == -1:  #
        return HttpResponse('{"status": 1, "message": "获取成功！", "data": [], "total": -1}')
    # print(soup['items'][0])
    for index, items in enumerate(htmls['items']):
        ...
        for item in soup.find_all(name='div', attrs={'class': 'content'}):
            ...
            if type in ['music', 'movie', 'book']:
                ...
                reviews['id'] = id      # 唯一标识ID
                reviews['i'] = i        # 序号
                i += 1
                reviews['name'] = name  #名字
                reviews['type'] = type  #类型
                reviews['link'] = link  #链接
                reviews['star'] = star  #评价星星
                reviews['score'] = score    #评分
                reviews['author'] = author  #作者
                reviews['reviews_count'] = reviews_count    #评价人数
                reviews['brief'] = brief    #简述
                reviews['img'] = img
        if not reviews:
            continue
        data.append(reviews)
    # more = htmls['more']        #标识是否还有下一页
    if htmls['more']:
        more = 1
    else:
        more = 0
    ...
    # return HttpResponse('{"status": 1, "message": "获取成功！", "data": ' + str(data) + ', "total": ' + str(total) + ', "more": ' +
    #         str(more) + '}')
    return JsonResponse(json.loads('{"status": 1, "message": "获取成功！", "data": ' + str(data) + ', "total": ' + str(total) + ', "more": ' +
            str(more) + '}'))
