...
from utils.mongodb_model import CollectMusicCommentsDB


# 存储某个电影所有评论
# def get_movie_comments(request):
def get_music_comments(id):
    result = CollectMusicCommentsDB.objects.filter(comments_music_id=id).count()
    # 若果存在记录，则不再进行爬取，直接return
    if (result and result >= 100):#短评只能获取到100条
        return result
    ...
    while True:
        ...
        # url = 'https://music.douban.com/subject/35030138/comments/new?p=2&_=1587693299777';
        # url = "http://127.0.0.1:8080/try/1/5/4.html"
        url = 'https://music.douban.com/subject/' + str(id) + '/comments/new?p=' + str(start) + '&_=' + str(tt)
        print(url)
        try:
            headers = {
            ...
            print('{"status": 0, "message": "获取失败！", "type": "get_music_comments" ,"url": ', url, ' , "music_id": ',str(id), '_', str(start), '}')
        ...
        if html.status_code == 404:
            print('{"status": 0, "message": "获取失败404！", "type": "get_music_comments" ,"url": "', url, '" ,"music_id": ',str(id), '_', str(start), '}')
            with codecs.open('record/' + datetime.datetime.now().strftime('%Y-%m-%d') + '_error.txt', 'a+','utf-8') as output:
        ...
        for item in soup.find_all(name='div', attrs={'class': 'comment'}):
            ...
            if star:
                ...
                star_str = ''
                star = 0
            if CollectMusicCommentsDB.objects.filter(comments_id=comments_id).first():
                continue
            else:
                i+=1
                CollectMusicCommentsDB.objects.create(
                    comments_id=comments_id,
                    comments_music_id=id,
